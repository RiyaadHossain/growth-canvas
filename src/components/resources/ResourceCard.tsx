...
export function ResourceCard({ item, index }: { item: ResourceItem; index: number }) {
  const inner = (
    <div className="group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
      {item.coverImage && (
...
export function FeaturedResourceCard({ item }: { item: ResourceItem }) {
  const inner = (
    <div className="group cursor-pointer overflow-hidden rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-transparent transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl">
      {item.coverImage && (
